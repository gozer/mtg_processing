...
def get_cards(http):
    ...
    for wanted_set in WANTS:
        for wanted_card in WANTS[wanted_set]:
            ...
            edition_name = WANTS[wanted_set][wanted_card]['wanted_edition']
    ...
    for set_code, set_data in sets.items():
        print(
            'Finding cards out of {} [{}]'.format(
                set_code, set_map[set_code][0]
            )
        )
        set_info = set_map.get(set_code)
        if not set_info:
            print(f"XXX: Can't find setinfo for {set_code}")
            continue
...
def wishlist_map(_inventory, card):
    # Count,Name,Edition,Card Number,Condition,Language,Foil,Signed,Artist
    # Proof,Altered Art,Misprint,Promo,Textless
    name = card['name']
...
def get_services(**options):
    return {
        ...
        'cards': create_engine(
            'mysql+pymysql://mtg:mtg@localhost/mtg', echo=False
        ),
    }
...
if __name__ == '__main__':
    ...
    with bonobo.parse_args(parser) as options:
        ...
        inventory_g = get_inventory_graph(**options)
        graph = get_graph(**options)

        bonobo.run(inventory_g, services=svc)
